...
COPY vsphere8-stig-auditor.ps1 ./
...
RUN echo '#!/usr/bin/pwsh' > /app/entrypoint.ps1 && \
    ...
    echo 'if (-not $VCenter) { Write-Host "Usage: docker run vsphere8-stig-auditor -VCenter vcenter.company.com"; exit 1 }' >> /app/entrypoint.ps1 && \
    ...
    echo '& /app/vsphere8-stig-auditor.ps1 @params' >> /app/entrypoint.ps1 && \
